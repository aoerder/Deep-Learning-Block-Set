function page = dlbsHelpBlockGain()

page = dlbsHelpPageTemplate("dlbsHelpBlockGain", "Gain");

page.addTitle("Gain");
page.setBlockLibraryPath("dlbsBlockGain/Gain")
page.addBlockImage("block")
page.addParagraphAndMaskHelp("Scales the input tensor by a gain parameter. The operation is applied element-wise across all tensor elements.");
...
parameters(1).Name = "Gain";
parameters(1).Parameter = "g";
parameters(1).Size = "Scalar";
parameters(1).Type = "Numeric";
parameters(1).Description = "Gain value multiplied with each element of X.";
page.addTable(struct2table(parameters))

page.addTitle("Mathematical Description");
page.addParagraph("*Forward*");
page.addParagraph("$$Y = gX$$")
