classdef dlbsTestSqrt < dlbsAbstractHarnessTest
    ...
    methods (Test)
        ...
        function testCalc(testCase)
            ...
            % Setup nonnegative inputs for element-wise sqrt
            inputs.X = abs(rand(3,3));
            inputs.dY = randn(3,3);

            % Simulate the model
            testCase.setHarness("dlbsTestHarnessSqrt");
            ...
            % Verify
            testCase.verifyEqual(output.Y,expected.Y, "RelTol",10*eps());
            testCase.verifyEqual(output.dX,expected.dX, "RelTol",10*eps());
        end
    end
