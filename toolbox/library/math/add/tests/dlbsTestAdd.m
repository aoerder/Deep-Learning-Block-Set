classdef dlbsTestAdd < dlbsAbstractHarnessTest


    methods (Test)

        function testCalcNoBroadcast(testCase)
            ...
            % Simulate the model
            testCase.setHarness("dlbsTestHarnessAdd");
        ...
        function testCalcBroadcasted(testCase)

            % Setup inputs for 2x1 + 1x3 add (broadcasted)
            inputs.X1 = randn(1,3);
            inputs.X2 = randn(2,1);
            inputs.dY = randn(2,3);

            % Simulate the model
            testCase.setHarness("dlbsTestHarnessAdd_broadcasted");
            testCase.harness = testCase.harness.setVariable('X1',  inputs.X1);
            testCase.harness = testCase.harness.setVariable('X2',  inputs.X2);
            testCase.harness = testCase.harness.setVariable('dY',  inputs.dY);
            simout = sim(testCase.harness);
