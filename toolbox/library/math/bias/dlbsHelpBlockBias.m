function page = dlbsHelpBlockBias()

page = dlbsHelpPageTemplate("dlbsHelpBlockBias", "Bias");

page.addTitle("Bias");
page.setBlockLibraryPath("dlbsBlockBias/Bias")
page.addBlockImage("block")
page.addParagraphAndMaskHelp("Adds a bias parameter to the input tensor. The operation is applied element-wise across all tensor elements.");

page.addTitle("Inputs and Outputs");
io = struct.empty;
io(1).Port = "X";
io(1).Type = "Input (passes gradient)";
io(1).Size = "(d1,d2,...,dn)";
io(1).Description = "Input tensor.";
io(2).Port = "Y";
io(2).Type = "Output (receives gradient)";
io(2).Size = "(d1,d2,...,dn)";
io(2).Description = "Output tensor.";
page.addTable(struct2table(io))

page.addTitle("Parameters");
page.addParagraph("This block exposes the following parameter:");

parameters = struct.empty;
parameters(1).Name = "Bias";
parameters(1).Parameter = "b";
parameters(1).Size = "Scalar";
parameters(1).Type = "Numeric";
parameters(1).Description = "Bias value added to each element of X.";
page.addTable(struct2table(parameters))

page.addTitle("Mathematical Description");
page.addParagraph("*Forward*");
page.addParagraph("$$Y = X + b$$")

if nargout == 0 % preview, if run with F5
    page.preview()
end

end
